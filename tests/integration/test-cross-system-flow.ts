...
async function testCliEmitShellAction(): Promise<void> {
  ...
  try {
    ...
    assert(events[0].processed_at === null, 'event is unprocessed before processing');

    const policies = ctx.loader.loadAll();
    await ctx.engine.processOnce(policies);

    const processed = getEvents(ctx.db);
    assert(processed[0].processed_at !== null, 'event marked processed after processOnce');
    ...
    assert(fs.existsSync(ACTION_LOG), 'shell action wrote to /tmp/nanoclaw-test-actions.log');
...
async function testEventChainBoiCompleted(): Promise<void> {
  ...
  try {
    ...
    const events = getEvents(ctx.db);
    const chainedEvent = events.find((e) => e.event_type === 'review.requested');
    assert(chainedEvent !== undefined, 'review.requested chained event exists');

    const payload = JSON.parse(chainedEvent!.payload as string) as Record<string, unknown>;
    assertEq(payload.spec_id as string, 'q-999', 'chained event has correct spec_id');
    assertEq(payload.source as string, 'auto-chain', 'chained event has source=auto-chain');

    const actions = getActionLogs(ctx.db);
    const emitAction = actions.find(
      (a) => a.policy_name === 'test-chain' && a.action_type === 'emit',
    );
    assert(emitAction !== undefined, 'action_log has emit action for test-chain');
...
async function testConditionFailureBlocksAction(): Promise<void> {
  ...
  try {
    ...
    const events = getEvents(ctx.db);
    const chainedEvent = events.find((e) => e.event_type === 'review.requested');
    assert(chainedEvent === undefined, 'NO review.requested event when status=failure');
...
async function testMultipleEventsInSequence(): Promise<void> {
  ...
  try {
    ...
    ctx.store.emit('boi.spec.completed', { spec_id: 'q-100', status: 'success' }, 'test');
    ...
    assertEq(events.length, 5, 'all 3 original events + 2 chained events = 5 total');
    ...
    assert(chainAction !== undefined, 'test-chain fired for boi.spec.completed');
...
async function testFullPipelineAudit(): Promise<void> {
  ...
  try {
    ...
    ctx.db
      ...
      .run('test.event', JSON.stringify({ marker: 'audit-test' }), 'audit-source');

    // Verify unprocessed
    const beforeProcess = getEvents(ctx.db);
    assertEq(beforeProcess.length, 1, 'one event before processing');
    assert(beforeProcess[0].processed_at === null, 'event unprocessed before processOnce');
    ...
    assert(afterProcess[0].processed_at !== null, 'processed_at set after processOnce');

    // Verify action_log
    const actions = getActionLogs(ctx.db);
    const echoAction = actions.find((a) => a.policy_name === 'test-echo');
    assert(echoAction !== undefined, 'action_log has test-echo entry');
    assertEq(echoAction!.rule_name as string, 'echo-on-test', 'action_log has correct rule_name');
    assertEq(echoAction!.status as string, 'success', 'action_log status=success');
    ...
    assertEq(echoEval!.trigger_matched as number, 1, 'eval_log trigger_matched=1');
  } finally {
    ctx.cleanup();
    cleanupActionLog();
  }
